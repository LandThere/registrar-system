...
$DbName = "archsys";
$backup_name = "main_archive(backup).sql";
$tables = array("main_archive");
...
function Export_Database($host, $user, $pass, $name, $tables = false, $backup_name = false)
{
    ...
    if (file_put_contents($save_path, $content) !== false) {
        ...
        header("Location: archivemain.php?status=success");
        exit();
    } else {
        // Redirect to archivemain.php with error message as a query parameter
        header("Location: archivemain.php?status=error");
        exit();
    }
