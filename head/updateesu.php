...
include '../config.php';
$id=$_GET['updateid'];
$select="Select * from `std_esu` where id=$id";
...
<form action="" method="post">
   ...
   <h3 style="color: red;"> REGISTRAR ARCHIVING</h3>
      ...
      <input type="number" name="std_id" placeholder="Enter Student ID" style="-moz-appearance: textfield; -webkit-appearance: textfield; appearance: textfield;" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" maxlength="10"  value=<?php echo $std_id?>>      <br>
...
if (isset($_POST['submit'])) {
    ...
    if (!empty($remarks) && preg_match('/^[a-zA-Z0-9]+$/', $remarks)) {
        $error[] = "Input any Remarks";
    }
    ...
    if (empty($error)) {
        ...
        $insertLogQuery = "INSERT INTO stdesu_logs (log_id, lname, fname, mname, started, college, courname)
                           VALUES ('$id','$lname', '$fname', '$mname', '$started', '$college', '$courname')";
        mysqli_query($conn, $insertLogQuery);
    }

    $updateQuery = "UPDATE `std_esu`
